...
def test_compose_preserves_api_key_from_env_file() -> None:
    compose = yaml.safe_load(
        (WORKSPACE_ROOT / "docker-compose.yml").read_text(encoding="utf-8")
    )
...
def test_dockerfile_installs_dependencies_before_local_project() -> None:
    dockerfile = (WORKSPACE_ROOT / "services/api/Dockerfile").read_text(
        encoding="utf-8"
    )
...
def test_live_announcement_source_uses_twenty_items_and_hourly_refresh() -> None:
    ...
    source = next(
        item for item in payload["sources"] if item["name"] == "nptu-overview"
    )
